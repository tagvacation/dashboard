...
export default function LibraryPage() {
  ...
  // Initial load + auto-refresh while any story is still generating
  useEffect(() => {
    let cancelled = false
    ...
    const tick = async () => {
      try {
        ...
        setStories(stories)
        setLoading(false)
        const anyGenerating = stories.some((s: Story) => ['generating', 'init', 'topic', 'script', 'audio', 'veo_submit', 'veo_poll'].includes(s.status))
        if (anyGenerating) timer = setTimeout(tick, 5000)
      } catch {
    ...
    <div className="px-4 md:px-8 py-8 max-w-6xl mx-auto">
      ...
      <div className="mb-6 flex flex-col md:flex-row md:items-end justify-between gap-4">
        ...
        <div className="flex gap-2">
          <Link href="/ads"
            className="px-4 py-2 bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 text-white rounded-xl text-sm font-semibold transition-colors shadow-lg">
            🎤 New Ad
          </Link>
          <Link href="/generate"
            className="px-4 py-2 bg-white hover:bg-white/90 text-black rounded-xl text-sm font-semibold transition-colors">
            ✦ New Story
          </Link>
        </div>
      </div>
      ...
      {loading ? (
        <div className="space-y-2">
          {[1, 2, 3, 4].map(i => (
            <div key={i} className="h-20 bg-white/[0.03] border border-white/10 rounded-2xl animate-pulse" />
          ))}
        </div>
      ) : filtered.length === 0 ? (
        <div className="text-center py-20">
          <div className="w-16 h-16 rounded-2xl bg-white/5 flex items-center justify-center text-3xl mx-auto mb-4">📹</div>
          <p className="text-sm font-semibold">No stories match</p>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-2 flex-wrap">
                      ...
                      {isAd && (
                        <span className="text-xs px-1.5 py-0.5 bg-pink-500/20 text-pink-300 border border-pink-500/30 rounded-full font-medium">🎤 Ad</span>
                      )}
                      <span className="text-xs text-white/30">{story.clips?.length || 0} clips</span>
