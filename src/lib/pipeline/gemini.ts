...
export async function pickTopic(storyId: string): Promise<{ topic: string; theme: string; hook_idea: string }> {
  const system = `You are a creative director for a Hindi YouTube Shorts channel called KathaKar.
...
export async function writeScript(storyId: string, topic: string, theme: string): Promise<Script> {
  const system = `You are a master Hindi storyteller writing short-form moral stories for YouTube Reels.
...
Generate 8-10 scenes (8 seconds each, 64-80 seconds total). Return ONLY the JSON object matching this schema exactly:
{
  ...
  // Normalize scenes — attach anchors and pad scene_num
...
export async function rewriteFilteredPrompt(
  topic: string,
  scene: Scene,
): Promise<string> {
  const token = await getAccessToken()
  const res = await fetch(`${BASE}/${GEMINI_MODEL}:generateContent`, {
    ...
    headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' },
    body: JSON.stringify({
      system_instruction: { parts: [{ text: `You rewrite a Veo video prompt for a Hindi moral story animation. The prompt was REJECTED by Veo's content filter.
RULES:
