...
export async function runMascotAdPipeline(storyId: string): Promise<void> {
  ...
  try {
    ...
    if (meta.imageGcsUri) {
      ...
      const editPrompt = `Turn THIS exact product into an adorable 3D cartoon mascot character. KEEP the product's real shape, proportions, colours and label text/branding intact — the mascot IS this product, just given big friendly eyes, a cute face and tiny little stubby arms & feet. CRITICAL: it is the PRODUCT itself as a character — NEVER a humanoid; NO human torso, NO long human legs, NO muscular body, NO superhero physique. Keep the product's own silhouette and size ratio (a bottle stays bottle-shaped, a tube stays tube-shaped). Personality: ${personality}. Premium Pixar-style, very cute, soft glossy 3D, clean soft studio background. Compose as a FULL vertical 9:16 portrait, the whole mascot centered with headroom, filling the frame.`
      try {
